...
class ProfileManager(UserManager):
    def same_program(self, user, program, university):
        ...
        search_vectors = SearchVector(
            StringAgg("university", delimiter=" "), weight="B", config="english"
        ) + SearchVector(
            StringAgg("program", delimiter=" "), weight="B", config="english"
        )
        ...
        qs = (
            ...
            .filter(
                (Q(is_active=True))
                & (
                    Q(university__unaccent__icontains=university)
                    & Q(program__unaccent__icontains=program)
                )
                | (Q(similarity_university__gte=0.8) & Q(similarity_program__gte=0.7))
                | (Q(similarity_program__gte=0.85)) & (Q(public=True))
            )
        ...
        if len(search_text.strip()) < 2:
            qs = (
                ...
                .filter(
                    ...
                    & (
                        ...
                        | Q(username__unaccent__icontains=search_text)
                    ),
                    trigram__gte=0.2,
                )
                .exclude(is_superuser=True)
            ...
            if len(search_text.split()) == 2:
                ...
                trigram = (
                    ...
                    + TrigramSimilarity("last_name", search_text)
                    + TrigramSimilarity("first_name", search_text)
                    + TrigramSimilarity("username", search_text_1)
                    + TrigramSimilarity("last_name", search_text_1)
                    + TrigramSimilarity("first_name", search_text_1)
                    + TrigramSimilarity("username", search_text_2)
                    + TrigramSimilarity("last_name", search_text_2)
                    + TrigramSimilarity("first_name", search_text_2)
                )
...
class SearchLogManager(models.Manager):
    def related_terms(self, term):
        ...
        search_vectors = SearchVector("search_text", weight="A", config="english")
        ...
        qs = (
            ...
            .annotate(count=Count("recent_searches"))
...
class ReportBase(models.Model):
    ...
    reason = models.CharField(
        max_length=250, choices=Reason.choices, default=Reason.NOT_INTERESTED
    )


class ReportUser(ReportBase):

    reported_obj = models.ForeignKey(
        Profile,
