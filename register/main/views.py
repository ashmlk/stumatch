...
def contact_us(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            )  # message to be sent to no-reply@domain.com
            message = Mail(
                from_email="JoinCampus no-reply <no-reply@joincampus.ca>",
                to_emails="joincampus.ca@gmail.com",
            ...
            messages.success(
                request,
                "Successfully submitted your form. Thanks for getting in touch with us!",
            )
            form = ContactForm
...
def get_notifications(request):
    ...
    read_count = (
        str(request.user.notifications.unread().count()) + " new notifications "
        if request.user.notifications.read().count() > 1
...
def signup(request):
    if request.user.is_authenticated:
        if request.user.is_active:
            return redirect("home:course-dashboard")
    if request.method == "POST":
...
def validate_and_check_signup(request):
    ...
    if username != None:
        ...
        data['suggested_usernames'] = generate_username(email=email)
        return JsonResponse(dict(data))
...
def user_login(request):
    ...
    if request.method == "POST":
        ...
        if user is not None:
            ...
            messages.error(
                request, "Sorry, the username or password you entered is not correct"
            )
            if vp == "md":
                url = reverse("login")
...
def update_user_email_on_verification(request):
    ...
    if request.method == "POST":
        ...
        return render(
            request,
            "account/custom_snippets/verification_sent.html",
            {"email": new_email},
        )
        ...
        data["form"] = render_to_string(
            "account/email/update_email.html", context, request=request
        )
...
def edit_profile(request):
    ...
    context = {"account_active": "setting-link-active"}
...
def edit_profile_data(request):
    ...
    context = {"form": form, "account_active": "setting-link-active"}
...
def add_university(request):
    ...
    if request.method == "POST":
        ...
        form = SetUniversityForm(instance=request.user)
    context = {"form": form, "su": success_url}
    data["html_form"] = render_to_string(
        "main/settings/university_update.html", context, request=request
    )
    return JsonResponse(data)
            ...
            if request.user.is_authenticated:
                try:
                    ...
                    data["new_url"] = reverse(
                        "main:update-image", kwargs={"hid": request.user.get_hashid()}
                    )
                    return JsonResponse(data)
                except Exception as e:
                    pass
    ...
    if request.method == "POST":
        ...
        if request.user == user:
            if request.user.is_authenticated:
                ...
                data["html_form"] = render_to_string(
                    "main/settings/image_delete_confirm.html", context, request=request
                )
    return JsonResponse(data)
    ...
    if request.method == "POST":
        ...
        if user == request.user:
            ...
            if choice != None:
                ...
                data["ranking_form"] = render_to_string(
                    "main/settings/ranking_form.html", context, request=request
                )
...
def search_settings(request):
    ...
    if request.method == "POST":
        ...
        if request.user.is_authenticated:
            ...
            data["search_settings_form"] = render_to_string(
                "main/settings/search_settings_input.html", request=request
            )
            return JsonResponse(data)
...
def notification_settings_all(request):
    ...
    if request.method == "POST":
        ...
        if request.user.is_authenticated:
            ...
            data["notifications_form"] = render_to_string(
                "main/settings/notifications/notifications_pause_all.html",
                request=request,
            )
            return JsonResponse(data)
        ...
        return render(
            request, "main/settings/notifications/notifications_all.html", context
        )
...
def notifications_post_all(request):
    ...
    if request.user.is_authenticated:
        ...
        if choice == "postalloff":
            if request.user.get_post_notify_all == True:
                ...
                request.user.save()
            data["notifications_form"] = render_to_string(
                "main/settings/notifications/notifications_post_all.html",
                request=request,
            )
            return JsonResponse(data)
        ...
        if choice == "postcommentson":
            if request.user.get_post_notify_comments == False:
                ...
                request.user.save()
            data["notifications_form"] = render_to_string(
                "main/settings/notifications/notifications_post_comments.html",
                request=request,
            )
            return JsonResponse(data)
        ...
        if choice == "postlikeson":
            if request.user.get_post_notify_likes == False:
                ...
                request.user.save()
            data["notifications_form"] = render_to_string(
                "main/settings/notifications/notifications_post_likes.html",
                request=request,
            )
            return JsonResponse(data)
    ...
    return render(
        request, "main/settings/notifications/notifications_post.html", context
    )
...
def notifications_blog_all(request):
    ...
    if request.user.is_authenticated:
        ...
        if choice == "blogalloff":
            if request.user.get_blog_notify_all == True:
                ...
                request.user.save()
            data["notifications_form"] = render_to_string(
                "main/settings/notifications/notifications_blog_all.html",
                request=request,
            )
            return JsonResponse(data)
        ...
        if choice == "blogcommentson":
            if request.user.get_blog_notify_comments == False:
                ...
                request.user.save()
            data["notifications_form"] = render_to_string(
                "main/settings/notifications/notifications_blog_comments.html",
                request=request,
            )
            return JsonResponse(data)
    ...
    return render(
        request, "main/settings/notifications/notifications_blog.html", context
    )
...
def notifications_friends_all(request):
    ...
    if request.user.is_authenticated:
        ...
        if choice == "friendrequestoff":
            if request.user.get_friendrequest_notify == True:
                ...
                request.user.save()
            data["notifications_form"] = render_to_string(
                "main/settings/notifications/notifications_friends_all.html",
                request=request,
            )
            return JsonResponse(data)
    ...
    return render(
        request, "main/settings/notifications/notifications_friends.html", context
    )
...
def deletion_menu(request):
    ...
    if request.user.is_authenticated:
        ...
        return render(
            request,
            "main/settings/deletion/menu.html",
            {"privacy_active": "setting-link-active"},
        )
    else:
        return redirect("main:settings-edit")
...
def deletion_account(request):
    ...
    if request.user.is_authenticated:
        ...
        return render(
            request,
            "main/settings/deletion/account.html",
            {"form": form, "privacy_active": "setting-link-active"},
        )
...
def bookmarks(request):
    ...
    if o == "posts":
        ...
        context = {"bookmarked_posts": bookmarked_posts, "post_active": post_active}
    # elif o == "blogs":
    #     blog_active = "-active"
    #     bookmarked_blogs = user.bookmarkblog_set.all()
    #     context = {"bookmarked_blogs": bookmarked_blogs, "blog_active": blog_active}
...
def f_post_tag(request, slug):
    ...
    if request.method == "POST":
        if user.favorite_post_tags.filter(slug=slug).exists():
            ...
            is_fav = True
        data["html_form"] = render_to_string(
            "main/tags/fav_post.html", {"is_fav": is_fav, "tag": tag}, request=request
        )
        return JsonResponse(data)
...
def f_blog_tag(request, slug):
    ...
    if request.method == "POST":
        if user.favorite_blog_tags.filter(slug=slug).exists():
            ...
            is_fav = True
        data["html_form"] = render_to_string(
            "main/tags/fav_blog.html", {"is_fav": is_fav, "tag": tag}, request=request
        )
        return JsonResponse(data)
...
def unblock_user(request, hid):
    ...
    will_be_unblock = Profile.objects.get(
        id=id
    )  # user to be unblocked by request.user (SIGNED IN USER)
    """ After blocking a user update the session holding the blockers_id """
    blockers_list = Block.objects.blocked(request.user)
    request.session["blockers"] = [u.id for u in blockers_list]
    if request.method == "POST":
...
def friends_main(request):
    ...
    for f in friends_list:
        ...
        mutual_friends_with_friends[f.username] = len(
            set(friends_list) & set(friends_friends)
        )
...
def friend_requests(request):
    ...
    for f in friend_requests_list:
        friends_friends = Friend.objects.friends(user=f)
        mutual_friends_with_requests[f.username] = len(
            set(friends_list) & set(friends_friends)
        )
...
def user_tags(request, username=None):
    ...
    num_obj = (
        request.user.favorite_post_tags.count()
        + request.user.favorite_blog_tags.count()
    )
    if num_obj < 1:
        s = "Tag"
    if o == "post":
...
def get_user(request, username, obj=None):
    ...
    obj_list = objects = mutual_courses = pnum = cnum = num_friends = user_courses = None
    page = request.GET.get('page', 1)
    ...
    if page == 1:
        ...
        num_friends = len(Friend.objects.friends(user))
        if user != request.user and obj == None:
            request_user_courses = request.user.courses.all().values("course_code","course_instructor_slug","course_university_slug").distinct("course_code","course_instructor_slug","course_university_slug")
            user_courses = user.courses.all().values("course_code","course_instructor_slug","course_university_slug").distinct("course_code","course_instructor_slug","course_university_slug")
            mutual_courses = request_user_courses & user_courses
    ...
    if request.user == user or Friend.objects.are_friends(request.user, user) or user.public:
        if obj == None:
            ...
            obj_list = user.post_set.all().order_by('-last_edited')
            
        elif user == request.user and obj == "bookmarks":
            obj_list= user.bookmarkpost_set.all().order_by('-obj__last_edited')
        ...
        if request.user == user:
            ...
            elif not Friend.objects.are_friends(request.user, user) and user.public:
                context["requested"] = context["pending"] = context["addfriend"] = False
                if FriendshipRequest.objects.filter(
    ...
    if Profile.objects.filter(id=user_id).exists():
        ...
        if request.method == "POST":
            if obj_type == "u":
                ...
                if form.is_valid():
                    if Profile.objects.filter(
                        id=hashids_user.decode(obj_id)[0]
                    ).exists():
                        report = form.save(False)
                        report.reporter = reporter
                        report.reported_obj = Profile.objects.get(
                            id=hashids_user.decode(obj_id)[0]
                        )
                        report.save()
            elif obj_type == "p":
                ...
                if form.is_valid():
                    if Post.objects.filter(id=hashids.decode(obj_id)[0]).exists():
                        report = form.save(False)
                        report.reporter = reporter
                        report.reported_obj = Post.objects.get(
                            id=hashids.decode(obj_id)[0]
                        )
                        report.save()
            elif obj_type == "cmnt":
                ...
                if form.is_valid():
                    if Comment.objects.filter(id=hashids.decode(obj_id)[0]).exists():
                        report = form.save(False)
                        report.reporter = reporter
                        report.reported_obj = Comment.objects.get(
                            id=hashids.decode(obj_id)[0]
                        )
                        report.save()
            elif obj_type == "blg":
                ...
                if form.is_valid():
                    if Blog.objects.filter(id=hashids.decode(obj_id)[0]).exists():
                        report = form.save(False)
                        report.reporter = reporter
                        report.reported_obj = Blog.objects.get(
                            id=hashids.decode(obj_id)[0]
                        )
                        report.save()
                ...
                if form.is_valid():
                    if BlogReply.objects.filter(id=hashids.decode(obj_id)[0]).exists():
                        report = form.save(False)
                        report.reporter = reporter
                        report.reported_obj = BlogReply.objects.get(
                            id=hashids.decode(obj_id)[0]
                        )
                        report.save()
            elif obj_type == "cr":
                ...
                if form.is_valid():
                    if Review.objects.filter(id=hashids.decode(obj_id)[0]).exists():
                        report = form.save(False)
                        report.reporter = reporter
                        report.reported_obj = Review.objects.get(
                            id=hashids.decode(obj_id)[0]
                        )
                        report.save()
            ...
            form_dict = {
                "u": ReportUserForm,
                "p": ReportPostForm,
                "cmnt": ReportCommentForm,
                "blg": ReportBlogForm,
                "blgrply": ReportBlogReplyForm,
                "cr": ReportCourseReviewForm,
            }
            ...
            data["html_form"] = render_to_string(
                "report/report_form.html", context, request=request
            )
    else:
...
def get_user_mentions(request):
    ...
    top_users_json = list(
        top_users[:10].values("first_name", "last_name", "username", "image")
    )
